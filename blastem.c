		...
		if (argv[i][0] == '-') {
			...
			case 'h':
				info_message(
					...
					"	-n          Disable Z80\n"
					"	-M SPEC     Genix MIDI dongle on a controller port (SPEC: a frame\n"
					"	            stream file, midi:<path> for raw MIDI bytes; options\n"
					"	            ,port=1 ,latency=<us> ,keys=on|off ,trace=<n>)\n"
					"	-C SPEC     Genix production cart mapper (SPEC: a card image or none;\n"
					"	            ,dip=1 ,dump=<path>[:<bytes>] ,trace=<n> ,eject=<frames>\n"
					"	            ,nosave ,save=<path>; the image is written back at exit\n"
					"	            unless nosave, like a battery save)\n"
